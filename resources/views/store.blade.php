...
@section('content')
    ...
    <div class="row row-cols-1 row-cols-md-3 g-4">
        @foreach ($products as $product)
            <div class="col">
                <div class="card">
                    <img src="{{ $product->image_path ? asset('product_image/' . $product->image_path) : 'https://placehold.co/200x200?text=No+Image' }}"
                            ...
                            <div class="modal-dialog modal-dialog-centered modal-sm">
                                <div class="modal-content">
                                    <div class="modal-header border-bottom-0 pb-0">
                                        <h5 class="modal-title fs-6 fw-bold text-truncate"
                                            id="addToCartModalLabel{{ $product->id }}">{{ $product->name }}</h5>
                                    ...
                                    <form action="{{ route('add_to_cart', $product->id) }}" method="POST">
                                        ...
                                        <div class="modal-body text-center pt-2">
                                            <p class="text-muted small mb-3">Available Stock: <strong
                                                    class="{{ $product->stock < 5 ? 'text-danger' : 'text-success' }}">{{ $product->stock }}</strong>
                                            </p>
                                            <div class="input-group mb-4 mx-auto" style="max-width: 140px;">
                                                <button class="btn btn-outline-secondary px-3" type="button"
                                                    onclick="const input = this.nextElementSibling; if(input.value > 1) input.value--">-</button>
                                                <input type="number" name="quantity"
                                                    class="form-control text-center bg-white px-1" value="1"
                                                    min="1" max="10" readonly>
                                                <button class="btn btn-outline-secondary px-3" type="button"
                                                    onclick="const input = this.previousElementSibling; const maxQty = {{ min(10, max(1, (int) $product->stock)) }}; if(input.value < maxQty) input.value++">+</button>
                                            </div>
